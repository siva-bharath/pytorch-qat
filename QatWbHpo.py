...
def train(config=None):
    """
    Main training function that will be called by wandb sweep
    """
    with wandb.init(config=config):
        ...
        model.classifier[0] = nn.Dropout(config.dropout)
        ...
        for epoch in range(1, config.epochs + 1):
            train_acc = train_one_epoch(
                model, criterion, optimizer, data_loader, device, epoch, scheduler
            )
